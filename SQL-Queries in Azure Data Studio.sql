...
WITH CTE_SevereCase (location, date, tot_hosp_adm, tot_icu_adm, tot_case, tot_death, population)
AS(
SELECT 
	   hi.location,
       hi.date,
       SUM(weekly_hosp_admissions/7) OVER (PARTITION BY hi.location ORDER BY hi.location , hi.date),
	   SUM(hi.weekly_icu_admissions/7) OVER (PARTITION BY hi.location ORDER BY hi.location , hi.date),
       SUM(cd.new_cases) OVER (PARTITION BY cd.location ORDER BY cd.location , cd.date),
       SUM(cd.new_deaths) OVER (PARTITION BY cd.location ORDER BY cd.location , cd.date),
       cd.population
FROM covid.hosp_icu  hi
JOIN covid.case_death  cd
